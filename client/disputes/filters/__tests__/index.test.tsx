...
async function addCurrencyFilter( filter: string ) {
	await user.click(
		screen.getByRole( 'button', { name: /All currencies/i } )
	);
	await user.click( screen.getByRole( 'button', { name: filter } ) );
}
...
describe( 'Dispute filters', () => {
	...
	describe( 'Needs response', () => {
		test( 'selecting the Needs response filter adds query param', async () => {
			updateQueryString( {}, '/', {} );
	...
	describe( 'Advanced', () => {
		...
		describe( 'when filtering by date', () => {
			...
			beforeEach( async () => {
				await addAdvancedFilter( 'Disputed on date' );
		...
		describe( 'when filtering by currency', () => {
			...
			test.each( [ 'usd', 'eur' ] )(
				'should filter by %s',
				async ( currency ) => {
					await addCurrencyFilter( formatCurrencyName( currency ) );
